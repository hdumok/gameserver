...
var Client = function (app, config)
{
    ...
    wsClient.once('connectFailed', function(error) {
        console.log('客户端连接服务器失败。 服务器地址:' + self.url + ' 服务器协议:' + self.protocol + ' 原因: ' + error.toString());
    ...
    wsClient.on('connect', function(connection) {
        ...
        connection.on('error', function(error) {
            console.log('客户端连接服务器过程中出错。 服务器地址:' + self.url +' 原因: ' + error.toString());
            app.invoke(self.connectionError, app, connection, error);
        });

        connection.on('close', function(code, reason) {
            console.log('客户端连接服务器关闭。 服务器地址:' + self.url +' 代码: ' + code + ' 原因: ' + reason);
